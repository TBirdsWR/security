...
function sm2WithSm4Encrypt(pk, data) {
  const sm4Key = getSm4Key()
  console.log(sm4Key)
  const encryptData = SM4.sm4Encrypt(_.fillSm4Data(data), sm4Key)

  const encryptSm4Key = doEncrypt(sm4Key, pk, 1)
  return {encryptData, encryptSm4Key}
}

function sm2WithSm4Decrypt(vk, encryptData, encryptSm4Key) {
  const sm4Key = doDecrypt(encryptSm4Key, vk, 1)
  console.log(sm4Key)
  return _.analysisSm4Data(SM4.sm4Decrypt(encryptData, sm4Key))
}

function getSm4Key() {
  const arr = []
  for (let i = 0; i < 16; i++) {
    arr.push(-128 + Math.floor(Math.random() * 255))
  }
  return _.arrayToHex(arr)
}


function getSm3Hash(data) {
  const smDigest = new SM3Digest()

  const z = new SM3Digest().getZ(G, data)
  const zValue = _.hexToArray(_.arrayToHex(z).toString())
...
module.exports = {
  sm4Encrypt: SM4.sm4Encrypt,
  sm4Decrypt: SM4.sm4Decrypt,
  ...
  doVerifySignature,
  getPoint,
  getGlobalCurve: _.getGlobalCurve,
  generateEcparam: _.generateEcparam,
  parseUtf8StringToHex: _.parseUtf8StringToHex,
  parseArrayBufferToHex: _.parseArrayBufferToHex,
  leftPad: _.leftPad,
  fillSm4Data: _.fillSm4Data,
  analysisSm4Data: _.analysisSm4Data,
  arrayToHex: _.arrayToHex,
  arrayToString: _.arrayToString,
  hexToArray: _.hexToArray,
  stringToArr: _.stringToArr,
  sm2WithSm4Encrypt,
  sm2WithSm4Decrypt,
  getSm3Hash,
}
