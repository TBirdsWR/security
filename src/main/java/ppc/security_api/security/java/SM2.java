...
public class SM2 {
	...
	public byte[] sm2GetZ(byte[] userId, ECPoint userKey){
		...
		p = Util.byteConvert32Bytes(userKey.normalize().getAffineXCoord().toBigInteger());
		sm3.update(p, 0, p.length);

		p = Util.byteConvert32Bytes(userKey.normalize().getAffineYCoord().toBigInteger());
		sm3.update(p, 0, p.length);
	...
	public void sm2Sign(byte[] md, BigInteger userD, ECPoint userKey, SM2Result sm2Result) {
		...
		do {
			do {
		        ...
		        System.out.println("计算曲线点X1: " + kp.normalize().getAffineXCoord().toBigInteger().toString(16));
				System.out.println("计算曲线点Y1: " + kp.normalize().getAffineYCoord().toBigInteger().toString(16));
				System.out.println("");

				// r
				r = e.add(kp.normalize().getAffineXCoord().toBigInteger());
				r = r.mod(ecc_n);
	...
	public void sm2Verify(byte md[], ECPoint userKey, BigInteger r, BigInteger s, SM2Result sm2Result) {
        ...
        if(t.equals(BigInteger.ZERO)) {
            ...
            System.out.println("计算曲线点X0: " + x1y1.normalize().getAffineXCoord().toBigInteger().toString(16));
			System.out.println("计算曲线点Y0: " + x1y1.normalize().getAffineYCoord().toBigInteger().toString(16));
			System.out.println("");

            x1y1 = x1y1.add(userKey.multiply(t));
            System.out.println("计算曲线点X1: " + x1y1.normalize().getAffineXCoord().toBigInteger().toString(16));
			System.out.println("计算曲线点Y1: " + x1y1.normalize().getAffineYCoord().toBigInteger().toString(16));
			System.out.println("");
            sm2Result.R = e.add(x1y1.normalize().getAffineXCoord().toBigInteger()).mod(ecc_n);
